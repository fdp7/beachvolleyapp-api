...
func main() {
	...
	secured := router.Use(auth.Auth())
	{
		...
		//secured.GET("/league/:username", league.GetUserLeagues)
		//secured.GET("/league/:leagueId", league.GetLeagueUsers)
		secured.POST("/league/:leagueId/add-user", league.AddUserToLeague)
		secured.DELETE("/league/:leagueId/delete-user", league.RemoveUserFromLeague)
